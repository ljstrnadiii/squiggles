...
- Unified desktop and compact navigation around a current-query dropdown rather than a visible tab strip or hamburger. Saved-query selection, query creation/settings, Statistics, Table, Rendering, dataset selection, AI Skills, and System settings share that menu; the header retains only the query control and right-aligned state dot. The three data views expose persisted size sliders for desktop drawers and mobile bottom sheets. New browsers default to miles while explicit URL and locally stored unit preferences continue to win.
