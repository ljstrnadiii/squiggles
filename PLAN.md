...
- Unified desktop and compact navigation around a current-query dropdown rather than a visible tab strip or hamburger. Saved-query selection, query creation/settings, dataset selection, AI Skills, and System settings share that menu; Statistics, Table, and Rendering remain icon controls at the right and expose persisted size sliders for desktop drawers and mobile bottom sheets. New browsers default to miles while explicit URL and locally stored unit preferences continue to win.
