...
The default output Hive-partitions by `activity_family`; `start_year` and `start_month` remain available to DuckDB as normal columns. Inside each family, the compiler Morton-orders route bbox centers across all years, writes 16-row Parquet groups inside browser-manageable shards, and records each shard's conservative covering bbox in the manifest. Use `--target-shard-rows` to tune physical shard size while keeping those logical partitions intact.
...
**Statistics**, **Table**, and **Rendering** are icon controls at the right of the shared desktop/mobile header; each opens as a resizable right drawer on desktop and a resizable bottom sheet on mobile. Statistics shows totals, averages, activity days, cleaning counts, and activity-family counts. Table loads lightweight metadata and bounds for every activity in the current SQL selection, sorts by activity, sport, date, distance, gain, or maximum elevation, and zooms to a route before opening its full detail. Rendering reports candidate/total shards, the actual LOD/raw representation, zoom, route and vertex counts, geometry latency, line width, heat work, data view, and basemap. The current query opens one dropdown for saved-query selection, query creation/settings, dataset selection, AI Skills, and System settings. Theme and units persist locally; display units default to miles unless a stored preference or deep link selects kilometres. Canonical SQL units remain metres and seconds.
