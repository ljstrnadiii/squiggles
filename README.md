...
**Statistics**, **Table**, and **Rendering** are first-class controls in the top header; each opens as a flush right drawer on desktop and a bottom sheet on mobile. Statistics shows totals, averages, activity days, cleaning counts, and activity-family counts. Table loads lightweight metadata and bounds for every activity in the current SQL selection, sorts by activity, sport, date, distance, gain, or maximum elevation, and zooms to a route before opening its full detail. Rendering reports candidate/total shards, the actual LOD/raw representation, zoom, route and vertex counts, geometry latency, line width, heat work, data view, and basemap. On compact screens, saved queries and secondary destinations move into the hamburger menu while the current query, dataset, and view controls use space-saving labels or icons. Theme and units live under **System settings** and persist locally; display units default to miles unless a stored preference or deep link selects kilometres. Canonical SQL units remain metres and seconds.
