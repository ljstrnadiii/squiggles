...
export function AccountPanel({ onClose, onIdentityChange, view = "account" }: { onClose: () => void; onIdentityChange: (completedLogin?: boolean) => void; view?: "account" | "upload" | "login" }) {
  ...
  return <section className="system-settings utility-panel account-panel" aria-label="Account">
    ...
    {profile && <>
      ...
      {profile.status === "approved" && view === "account" && <><section className="account-stats"><div><strong>{profile.stats?.activityCount.toLocaleString() ?? "0"}</strong><span>processed activities</span></div><div><strong>{profile.stats?.datasetCount.toLocaleString() ?? "0"}</strong><span>processed datasets</span></div><div><strong>{bytes(profile.stats?.curatedBytes ?? 0)}</strong><span>curated data</span></div><div><strong>{bytes(profile.stats?.uploadedBytes ?? 0)}</strong><span>uploaded source</span></div><div><strong>{profile.stats?.publishedViews.toLocaleString() ?? "0"}</strong><span>map views</span></div><div><strong>{profile.stats?.publishedMaps.toLocaleString() ?? "0"}</strong><span>saved views</span></div></section>{profile.compile && !["ready", "failed"].includes(profile.compile.status) && <div className="upload-row compile-progress"><span><strong>Compiling {profile.compile.filename}</strong>{profile.compile.statusDetail && <small>{profile.compile.statusDetail}</small>}</span><strong>{compilePercent(profile.compile.progressCompleted, profile.compile.progressTotal) !== null ? `${compilePercent(profile.compile.progressCompleted, profile.compile.progressTotal)}%` : profile.compile.status}</strong></div>}</>}
