...
import { listAdminUsers, retryAdminUpload, setAdminUserAccess, type AdminUser } from "./admin";
import "./admin.css";
...
export function AccountPanel({ onClose, onIdentityChange, view = "account" }: { onClose: () => void; onIdentityChange: () => void; view?: "account" | "upload" | "login" }) {
  ...
  const retryUpload = async (user: AdminUser) => {
    if (!config || !session || user.latestUpload?.status !== "failed") return;
    setAdminLoading(true); setError("");
    try { await retryAdminUpload(config, session, user.subject, user.latestUpload.id); setAdminUsers(await listAdminUsers(config, session)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Could not retry upload."); }
    finally { setAdminLoading(false); }
  };
