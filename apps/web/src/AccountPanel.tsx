...
export function AccountPanel({ onClose, onIdentityChange, view = "account" }: { onClose: () => void; onIdentityChange: (completedLogin?: boolean) => void; view?: "account" | "upload" | "login" }) {
  ...
  const compilePercent = (completed: number, total: number) => total > 0 ? Math.max(0, Math.min(100, Math.round(completed / total * 100))) : null;
  const recompilableUploadStatuses = new Set(["failed", "ready", "completed"]);
  const ownAdminUser = profile?.role === "admin" && profile.email
    ? adminUsers.find(user => user.email.toLowerCase() === profile.email?.toLowerCase())
    : undefined;
  ...
  const recompileUpload = async (user: AdminUser) => {
    if (!config || !session || !user.latestUpload) return;
    setAdminLoading(true); setError("");
    try { await recompileAdminUpload(config, session, user.subject, user.latestUpload.id); setAdminUsers(await listAdminUsers(config, session)); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Could not recompile upload."); }
    finally { setAdminLoading(false); }
  };
  const recompileOwnArchive = async () => {
    if (!ownAdminUser?.latestUpload) return;
    await recompileUpload(ownAdminUser);
  };
  ...
  return <section className="system-settings utility-panel account-panel" aria-label="Account">
    ...
    {profile && <>
      ...
      {profile.status === "approved" && view === "account" && <><section className="account-stats"><div><strong>{profile.stats?.activityCount.toLocaleString() ?? "0"}</strong><span>processed activities</span></div><div><strong>{profile.stats?.datasetCount.toLocaleString() ?? "0"}</strong><span>processed datasets</span></div><div><strong>{bytes(profile.stats?.curatedBytes ?? 0)}</strong><span>curated data</span></div><div><strong>{bytes(profile.stats?.uploadedBytes ?? 0)}</strong><span>uploaded source</span></div><div><strong>{profile.stats?.publishedViews.toLocaleString() ?? "0"}</strong><span>map views</span></div><div><strong>{profile.stats?.publishedMaps.toLocaleString() ?? "0"}</strong><span>saved views</span></div></section>{ownAdminUser?.latestUpload && recompilableUploadStatuses.has(ownAdminUser.latestUpload.status) && <button className="account-primary" disabled={adminLoading} onClick={() => void recompileOwnArchive()}>Recompile latest archive</button>}{profile.compile && !["ready", "failed"].includes(profile.compile.status) && <div className="upload-row compile-progress"><span><strong>Compiling {profile.compile.filename}</strong>{profile.compile.statusDetail && <small>{profile.compile.statusDetail}</small>}</span><strong>{compilePercent(profile.compile.progressCompleted, profile.compile.progressTotal) !== null ? `${compilePercent(profile.compile.progressCompleted, profile.compile.progressTotal)}%` : profile.compile.status}</strong></div>}</>}
      {profile.status === "approved" && view === "upload" && <section className="account-upload"><p>Upload an activity archive. Strava ZIP exports are currently supported; media is skipped. We’ll email you once optimization finishes; that is the final automated email Squiggles sends.</p><label className={`archive-dropzone ${uploading ? "uploading" : ""}`} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); void selectArchive(event.dataTransfer.files[0]); }}><strong>{uploading ? `Uploading ${Math.round(progress * 100)}%` : "Drop an activity ZIP here"}</strong><span>or choose a file</span><input type="file" accept=".zip,application/zip" disabled={uploading} onChange={event => void selectArchive(event.target.files?.[0])} /></label>{uploads.map(upload => { const paused = pausedUpload === upload.filename && upload.status === "uploading"; return <div className="upload-row" key={upload.id}><span>{upload.filename}{paused ? <small>Upload interrupted. Select this ZIP again to resume.</small> : upload.statusDetail && <small>{upload.statusDetail}</small>}</span><strong>{paused ? "paused" : upload.status === "compiling" && compilePercent(upload.progressCompleted, upload.progressTotal) !== null ? `${compilePercent(upload.progressCompleted, upload.progressTotal)}%` : upload.status}</strong></div>; })}</section>}
      {profile.role === "admin" && view === "account" && <section className="admin-console"><header><strong>Admin · users</strong><span>{adminUsers.length} accounts</span></header><div className="admin-users">{adminUsers.length === 0 && <div className="admin-empty">No users yet.</div>}{adminUsers.map(user => <article className="admin-user" key={user.subject}><div className="admin-user-top"><div className="admin-user-identity"><strong>{user.name || user.email || user.subject}</strong><span>{user.email || user.subject}</span></div><span className={`admin-user-status ${user.status}`}>{user.status}</span></div><div className="admin-user-meta"><span>{user.phase}</span><span>{user.uploads} upload{user.uploads === 1 ? "" : "s"}</span><span>{user.activityCount.toLocaleString()} activities</span><span>{user.publishedViews.toLocaleString()} views</span></div>{user.latestUpload && <div className="admin-user-detail">Latest: {user.latestUpload.filename} · {user.latestUpload.status}{user.latestUpload.progressTotal > 0 ? ` · ${compilePercent(user.latestUpload.progressCompleted, user.latestUpload.progressTotal)}%` : ""}{user.latestUpload.statusDetail ? ` · ${user.latestUpload.statusDetail}` : ""}</div>}<div className="admin-user-links">{user.mapUrl && <a href={`${user.mapUrl}?admin=1`} target="_blank" rel="noreferrer">View map</a>}</div><div className="admin-user-actions">{user.role !== "admin" && <><button className={user.status === "approved" ? "active" : ""} disabled={adminLoading} onClick={() => void changeAccess(user, "approved")}>Grant access</button><button className={user.status === "pending" ? "active" : ""} disabled={adminLoading} onClick={() => void changeAccess(user, "pending")}>Pending</button><button className={user.status === "rejected" ? "active" : ""} disabled={adminLoading} onClick={() => void changeAccess(user, "rejected")}>Reject</button></>}{user.latestUpload && recompilableUploadStatuses.has(user.latestUpload.status) && <button disabled={adminLoading} onClick={() => void recompileUpload(user)}>Recompile</button>}</div></article>)}</div></section>}
