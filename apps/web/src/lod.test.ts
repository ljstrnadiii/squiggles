...
describe("chooseLod", () => {
  ...
  it("falls back to the lowest available level when every estimate exceeds budget", () => {
    expect(chooseLod([6000000, 15000000, 60000000, 250000000, 500000000], 0)).toBe(0);
  });

  it("uses full geometry at close zoom when it fits the viewport budget", () => {
    expect(chooseLod([100, 250, 1000, 5000, 24000], 4)).toBe(4);
    expect(chooseLod([100, 250, 1000, 5000, 24000], 3)).toBe(3);
  });

  it("does not read raw geometry for an over-budget dense close view", () => {
    expect(chooseLod([51400, 128500, 514000, 2200000, 4761806], 4)).toBe(2);
    expect(chooseLod([98000, 247100, 988400, 4440000, 8700000], 4)).toBe(2);
  });
});
