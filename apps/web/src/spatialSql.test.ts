...
describe("drawn spatial filters", () => {
  ...
  it("materializes only bbox-pruned candidate ids before exact geometry checks", () => {
    const result = applySpatialFilterSql(
      "SELECT activity_id FROM activities WHERE start_year = 2026",
      { predicate: "intersects", polygon, visible: false },
    );
  ...
  it("repeats bbox pruning on the current render geometry relation", () => {
    const result = applySpatialFilterSql("SELECT activity_id FROM activities", {
      predicate: "intersects",
      polygon,
      visible: false,
    });
  ...
  it("uses one polygon geometry and vectorized DuckDB Spatial conversion for exact checks", () => {
    const result = applySpatialFilterSql("SELECT activity_id FROM activities", {
      predicate: "intersects",
      polygon,
      visible: false,
    ...
    expect(result).toContain("AS POINT_2D");
    expect(result).toContain("AS LINESTRING_2D");
    expect(result).not.toContain("ST_MakeLine(");
    expect(result).not.toContain("ST_Point(");
  ...
  it("does not let display visibility change selection SQL", () => {
    const hidden = applySpatialFilterSql("SELECT activity_id FROM activities", {
      predicate: "intersects",
      polygon,
      visible: false,
    });
    const shown = applySpatialFilterSql("SELECT activity_id FROM activities", {
      predicate: "intersects",
      polygon,
      visible: true,
    });
