...
function usePanelSize(name: string, defaultWidth: number) {
  const key = `squiggles-panel-${name}`;
  const [size, setSize] = useState<PanelSize>(() => {
    try { return { width: defaultWidth, height: 72, ...JSON.parse(localStorage.getItem(key) ?? "{}") as Partial<PanelSize> }; }
    catch { return { width: defaultWidth, height: 72 }; }
  });
  function change(axis: keyof PanelSize, value: number) {
    setSize(current => {
...
function PanelSizeControl({ panel, compact }: { panel: ReturnType<typeof usePanelSize>; compact: boolean }) {
  const axis = compact ? "height" : "width";
  return <label className="panel-size-control">Size<input aria-label="Panel size" type="range" min="30" max="90" step="1" value={panel.size[axis]} onChange={event => panel.change(axis, Number(event.target.value))} /></label>;
}

function panelStyle(panel: ReturnType<typeof usePanelSize>): CSSProperties {
  return { "--panel-width": `${panel.size.width}vw`, "--panel-height": `${panel.size.height}vh` } as CSSProperties;
}
...
export function App() {
  ...
  const statsPanel = usePanelSize("statistics", 32);
  const tablePanel = usePanelSize("table", 68);
  const renderingPanel = usePanelSize("rendering", 32);
  const compactLayout = mapSize.width <= 600;
  ...
  return <main className="app" onKeyDown={event => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") void run(); }}>
    <header className="topbar">
      ...
      <button className="mobile-query-title" aria-label={menuOpen ? "Close query menu" : "Open query menu"} aria-expanded={menuOpen} onClick={() => { setMenuOpen(open => !open); setSystemSettingsOpen(false); }}>{tab.title}<span aria-hidden="true">⌄</span></button>
      <div className={`status ${busy ? "working" : ""}`} role="status" aria-label={status}><span /></div>
      <button className={`utility-button view-button ${statsOpen ? "active" : ""}`} aria-label="Statistics" title="Open detailed statistics for the current SQL selection" disabled={!selectionReady.current} onClick={toggleStats}><span aria-hidden="true">▥</span></button>
      <button className={`utility-button view-button ${tableOpen ? "active" : ""}`} aria-label="Table" title="Browse, sort, and navigate every activity in the current SQL selection" disabled={!selectionReady.current || tableLoading} onClick={() => void toggleTable()}><span aria-hidden="true">▤</span></button>
      <button className={`utility-button view-button ${renderingOpen ? "active" : ""}`} aria-label="Rendering" title="Inspect geometry fidelity and browser rendering work" disabled={!selectionReady.current} onClick={() => { setRenderingOpen(open => !open); setStatsOpen(false); setTableOpen(false); setAboutOpen(false); setToolbarOpen(false); setSelected(null); setProfileHover(null); setIsolateSelected(false); }}><span aria-hidden="true">≋</span></button>
    </header>
    ...
    {menuOpen && <nav className="mobile-menu utility-panel" aria-label="Query navigation">
      ...
      <section><button onClick={() => { choose(tab, true); setMenuOpen(false); }}>⌘ Query settings</button><button onClick={openSystemSettings}>⚙ System settings</button><button disabled={busy} onClick={() => { void openDirectory(); setMenuOpen(false); }}>◫ {datasetName ? `Change dataset · ${datasetName}` : "Open dataset"}</button><button onClick={() => { setSchemaOpen(true); setStatsOpen(false); setTableOpen(false); setRenderingOpen(false); setAboutOpen(false); setToolbarOpen(false); setMenuOpen(false); setSelected(null); }}>AI Skills</button></section>
    </nav>}
