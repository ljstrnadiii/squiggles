...
type ArrowData = { valueOffsets?: unknown; values?: unknown; stride: number; children: ArrowData[]; type?: { toString(): string } };
...
function elevationProfile(value: unknown) {
  ...
  for (const raw of value == null ? [] : Array.from(value as Iterable<Iterable<number | null>>)) {
    ...
    previous = position;
    if (elevation != null && Number.isFinite(Number(elevation))) samples.push({ distanceM, elevationM: Number(elevation), position });
  }
  return samples;
}
...
function detailRoute(row: Record<string, unknown>, geometryKey: string) {
  ...
  return { activityId: String(scalar(row.activity_id)), name: String(row.name), sportType: String(row.sport_type), startTime: row.start_time?.toString() ?? null, distanceM: scalar(row.distance_m) as number | null, elevationGainM: scalar(row.elevation_gain_m) as number | null, maxElevationM: scalar(row.max_elevation_m) as number | null, sourceUrl: row.source_url as string | null, path, fullPath: row.geometry == null ? path : coordinates(row.geometry), elevationProfile: elevationProfile(row.elevation_profile) };
}
  ...
  return table.batches.map(batch => {
    ...
    if (!fixedPairs) {
      for (let point = firstPoint; point < lastPoint; point += 1) {
        if (pairOffsets![point + 1] - pairOffsets![point] !== 2) throw new Error("Viewport GeoArrow coordinates must contain exactly longitude and latitude");
      }
    }
...
async function initialize() {
  ...
  const bundles: duckdb.DuckDBBundles = {
    mvp: { mainModule: new URL("@duckdb/duckdb-wasm/dist/duckdb-mvp.wasm", import.meta.url).href, mainWorker: new URL("@duckdb/duckdb-wasm/dist/duckdb-browser-mvp.worker.js", import.meta.url).href },
    eh: { mainModule: new URL("@duckdb/duckdb-wasm/dist/duckdb-eh.wasm", import.meta.url).href, mainWorker: new URL("@duckdb/duckdb-wasm/dist/duckdb-browser-eh.worker.js", import.meta.url).href },
  };
...
function viewportPredicate(bounds?: Bounds, clean = false): string {
  ...
  const longitude = west <= east ? `a.${prefix}xmax >= ${west} AND a.${prefix}xmin <= ${east}` : `(a.${prefix}xmax >= ${west} OR a.${prefix}xmin <= ${east})`;
...
function viewportContainmentPredicate(bounds?: Bounds, clean = false): string {
  ...
  const [west, south, east, north] = bounds;
  const prefix = clean ? "clean_" : "";
  const longitude = west <= east ? `a.${prefix}xmin >= ${west} AND a.${prefix}xmax <= ${east}` : `(a.${prefix}xmin >= ${west} OR a.${prefix}xmax <= ${east})`;
...
async function summarize(bounds: Bounds | undefined, clean: boolean) {
  ...
  if (!relation) return { activityCount: 0, distanceM: 0, elapsedSeconds: 0, movingSeconds: 0, elevationGainM: 0, elevationLossM: 0, minElevationM: null, maxElevationM: null, maxDistanceM: null, activeDays: 0, droppedJumpPoints: 0, droppedElevationPoints: 0, sportCounts: [], firstActivity: null, lastActivity: null };
  ...
  const sports = await connection!.query(`SELECT a.activity_family sport,count(*) activity_count FROM ${relation} a${join} WHERE ${where} GROUP BY a.activity_family ORDER BY activity_count DESC,sport`);
