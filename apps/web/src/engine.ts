...
import { activateRenderTab, clearRenderPlanHints, recordActiveRenderPlan, recordRenderPlan } from "./renderPlanHints";
...
export function formatDuckDBDiagnostic(body:object,error:unknown,extra:Record<string,unknown>={}):string{
  ...
  if(files.length){lines.push("",`Files (${files.length}):`,...files.slice(0,20).map(file=>`- ${file}`));if(files.length>20)lines.push(`- … ${files.length-20} more`);}
...
export class BrowserDuckDBEngine implements ExecutionEngine {
  ...
  private initialPlan(tab:QueryTab,zoom:number){if(tab.startingLod!=null&&!this.consumedPublishedLods.has(tab.id)){this.consumedPublishedLods.add(tab.id);return {lod:tab.startingLod,startingVertexEstimate:tab.startingVertexEstimate};}return {lod:this.requestedLod(zoom),startingVertexEstimate:undefined};}
  ...
  async openDataset(source:DatasetSource,onProgress?: (completed:number,total:number)=>void):Promise<Dataset>{
    ...
    this.datasetRevision+=1;this.selectionKey="";this.cache.clear();this.cacheBytes=0;this.cacheEvictions=0;this.consumedPublishedLods.clear();clearRenderPlanHints();
    const manifestStarted=performance.now();
    const manifest=source.kind==="directory"?JSON.parse(await(await(await source.handle.getFileHandle("dataset.json")).getFile()).text()) as DatasetManifest:await fetch(`${source.baseUrl}/dataset.json`).then(response=>{if(!response.ok)throw new Error(`Could not load dataset manifest (${response.status})`);return response.json() as Promise<DatasetManifest>;});
    ...
    const renderEntries=(manifest.render_levels??[]).flatMap(level=>level.files.map(file=>({lod:level.lod,file})));const totalEntries=manifest.shards.length+renderEntries.length;
    if(source.kind==="url"){for(const shard of manifest.shards)files.push(workerFile(shard));for(const level of manifest.render_levels??[])renderLevels.push({lod:level.lod,files:level.files.map(file=>workerFile(file))});onProgress?.(totalEntries,totalEntries);}
    else{let completed=0;const load=async(entry:DatasetFileManifest)=>{let directory=source.handle;const parts=entry.path.split("/");for(const part of parts.slice(0,-1))directory=await directory.getDirectoryHandle(part);const buffer=await(await(await directory.getFileHandle(parts.at(-1)!)).getFile()).arrayBuffer();onProgress?.(++completed,totalEntries);return workerFile(entry,buffer);};for(const shard of manifest.shards)files.push(await load(shard));for(const level of manifest.render_levels??[]){const levelFiles:WorkerFile[]=[];for(const entry of level.files)levelFiles.push(await load(entry));renderLevels.push({lod:level.lod,files:levelFiles});}}
    const workerStarted=performance.now();const name=source.kind==="directory"?source.handle.name:source.name;const allRenderFiles=renderLevels.flatMap(level=>level.files);const openRequest={type:"open",files,renderLevels,schemaVersion:manifest.schema_version};
    try{await this.request(openRequest,[...files,...allRenderFiles].flatMap(file=>file.buffer?[file.buffer]:[]));}catch(error){throw new Error(formatDuckDBDiagnostic(openRequest,error,{dataset:name,schemaVersion:manifest.schema_version,files:[...files.map(file=>file.name),...allRenderFiles.map(file=>file.name)]}));}
    const workerOpenMs=performance.now()-workerStarted;perf("dataset-open",{dataset:name,totalMs:Math.round(performance.now()-started),manifestMs:Math.round(manifestMs),workerOpenMs:Math.round(workerOpenMs),shards:files.length,renderLevels:renderLevels.length,renderFiles:allRenderFiles.length,activityCount:manifest.activity_count});return{id:name,name,manifest};
  }
  async execute(tab:QueryTab,zoom:number,bounds?:ViewportBounds):Promise<QueryResult&ViewportResult>{
    const started=performance.now();const nextClean=tab.style.cleanEnabled;const baseSql=normalizeSelectionSql(tab.sql);const sql=applySpatialFilterSql(baseSql,tab.spatialFilter);const plan=this.initialPlan(tab,zoom);
    ...
    this.clean=nextClean;this.selectionKey=`${this.datasetRevision}|${this.clean?1:0}|${sql}`;activateRenderTab(tab.id);recordRenderPlan(tab.id,{lod:result.lod,vertexEstimate:result.plannedVertexEstimate});
  ...
  async renderViewport(zoom:number,bounds:ViewportBounds):Promise<ViewportResult>{
    const requestedKey=this.cacheKey(zoom,bounds);const cached=this.cached(requestedKey,bounds,zoom);if(cached){recordActiveRenderPlan({lod:cached.lod,vertexEstimate:cached.plannedVertexEstimate});perf("viewport-cache-hit",{zoom:Number(zoom.toFixed(2)),lod:cached.lod,vertices:cached.vertexCount});return cached;}
    const fetchBounds=padViewportBounds(bounds,VIEWPORT_PREFETCH_FRACTION);const started=performance.now();const requestedLod=this.requestedLod(zoom);const result=await this.networkRequest<WorkerViewportResult>({type:"render",lod:requestedLod,budget:RESOLUTION_VERTEX_BUDGETS[this.resolution],bounds:fetchBounds,clean:this.clean});recordActiveRenderPlan({lod:result.lod,vertexEstimate:result.plannedVertexEstimate});
    perf("viewport-fetch",{totalMs:Math.round(performance.now()-started),zoom:Number(zoom.toFixed(2)),requestedLod,plannedLod:result.lod,vertices:result.vertexCount,geometryBytes:result.geometryBufferBytes,candidateBytes:result.scan.candidateBytes,expectedRowGroups:result.scan.expectedRowGroupCount,prefetchFraction:VIEWPORT_PREFETCH_FRACTION});return this.cacheResult(result,this.cacheKey(zoom,fetchBounds),false,fetchBounds,zoom);
  }
...
export function padViewportBounds(bounds:ViewportBounds,fraction=VIEWPORT_PREFETCH_FRACTION):ViewportBounds{const [west,south,east,north]=bounds;if(west>east)return bounds;const longitudePadding=(east-west)*fraction;const latitudePadding=(north-south)*fraction;return [Math.max(-180,west-longitudePadding),Math.max(-85,south-latitudePadding),Math.min(180,east+longitudePadding),Math.min(85,north+latitudePadding)];}
function binaryBytes(batches:BinaryRouteBatch[]):number{const buffers=new Set<ArrayBufferLike>();for(const batch of batches)for(const array of [batch.positions,batch.startIndices,batch.segmentActivityIndices])buffers.add(array.buffer);return [...buffers].reduce((total,buffer)=>total+buffer.byteLength,0);}
