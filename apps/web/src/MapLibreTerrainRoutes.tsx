...
function terrainStyle(basemap: Basemap, dark: boolean): maplibregl.StyleSpecification {
  const background = dark ? "#07100e" : "#edf2ef";
  const sources: maplibregl.StyleSpecification["sources"] = {
    terrain: {type: "raster-dem", tiles: [DEM], tileSize: 256, maxzoom: 14, encoding: "terrarium"},
  };
  const layers: maplibregl.LayerSpecification[] = [{id: "background", type: "background", paint: {"background-color": background}}];
  if (basemap !== "blank") {
...
function segmentBatch(batches: BinaryRouteBatch[], colors: Uint8Array[]): SegmentBatch {
  let segmentCount = 0;
  ...
  batches.forEach((batch, batchIndex) => {
    ...
    for (let route = 0; route < batch.startIndices.length - 1; route++) {
      ...
      const end = batch.startIndices[route + 1];
...
class BinaryTerrainLayer implements TerrainLayer {
  ...
  setData(data: SegmentBatch, widthPx: number) { this.pending = data; this.segmentCount = data.segmentCount; this.widthPx = widthPx; }

  onAdd(map: maplibregl.Map, gl: WebGL2RenderingContext) {
    const vs = compile(gl, gl.VERTEX_SHADER, `#version 300 es
precision highp float;
uniform vec2 u_tile_origin; uniform float u_tile_scale; uniform float u_half_width;
in vec2 a_start; in vec2 a_end; in vec4 a_color; out vec4 v_color;
void main(){bool atEnd=gl_VertexID==2||gl_VertexID==3||gl_VertexID==5; float side=(gl_VertexID==1||gl_VertexID==4||gl_VertexID==5)?1.0:-1.0; vec2 s=a_start*u_tile_scale-u_tile_origin; vec2 e=a_end*u_tile_scale-u_tile_origin; vec2 d=e-s; float n=max(length(d),1e-9); vec2 normal=vec2(-d.y,d.x)/n; vec2 p=(atEnd?e:s)+normal*side*u_half_width; gl_Position=vec4(p.x*2.0-1.0,1.0-p.y*2.0,0.0,1.0); v_color=a_color;}`);
...
precision highp float; in vec4 v_color; out vec4 fragColor; void main(){fragColor=v_color;}`);
    ...
    this.endpointBuffer = gl.createBuffer(); this.colorBuffer = gl.createBuffer();
    this.upload(gl); map.triggerRepaint();
  }
  ...
  update(map: maplibregl.Map, gl: WebGL2RenderingContext, data: SegmentBatch, widthPx: number) { this.setData(data, widthPx); this.upload(gl); map.triggerRepaint(); }
  render() {}
  renderToTile(gl: WebGL2RenderingContext, options: TerrainInput) {
    ...
    const tile = options.tileID; const scale = 2 ** tile.canonical.z; const ox = tile.canonical.x + (tile.wrap ?? 0) * scale;
  ...
  onRemove(_map: maplibregl.Map, gl: WebGL2RenderingContext) { if(this.endpointBuffer)gl.deleteBuffer(this.endpointBuffer); if(this.colorBuffer)gl.deleteBuffer(this.colorBuffer); if(this.program)gl.deleteProgram(this.program); }
}
...
export function MapLibreTerrainRoutes({view, basemap, dark, batches, colors, widthPx, onView, onInteraction}: {view: MapState; basemap: Basemap; dark: boolean; batches: BinaryRouteBatch[]; colors: Uint8Array[]; widthPx: number; onView: (view: MapState) => void; onInteraction: (active: boolean) => void}) {
  const container = useRef<HTMLDivElement>(null); const mapRef = useRef<maplibregl.Map | null>(null); const layerRef = useRef<BinaryTerrainLayer | null>(null); const data = segmentBatch(batches, colors);
  useEffect(() => {
    ...
    mapRef.current = map;
    const layer = new BinaryTerrainLayer(); layer.setData(data, widthPx); layerRef.current = layer;
  ...
  useEffect(() => { const map=mapRef.current, layer=layerRef.current; if(!map||!layer) return; layer.update(map, map.painter.context.gl as WebGL2RenderingContext, data, widthPx); }, [data, widthPx]);
  useEffect(() => { const map=mapRef.current; if(map && map.isStyleLoaded()) map.setStyle(terrainStyle(basemap,dark)); }, [basemap,dark]);
