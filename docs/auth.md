...
Verified completion submits the immutable image for that deployment to AWS Batch on Fargate. The task downloads only the filtered source, runs the shared `compile_strava` implementation, validates its output, and writes curated GeoParquet to a separate private ingested bucket. Its owner-scoped upload record advances through `queued`, `downloading`, `compiling`, `publishing`, and `ready`, or records a bounded `failed` detail. During compilation it persists completed activities, total activities, and percentage every five seconds; the account drawer polls the record on the same interval. Batch has no idle worker, uses a four-hour timeout, and runs one attempt so a bad source cannot create an automatic retry bill. Curated user data is not placed beneath the current unlisted public CloudFront prefix; authenticated private delivery remains a separate acceptance stage.

After the dataset and its `ready` metadata are durable, the task sends a plain-text completion email from `notifications@squiggles.io` to the verified account email. Email failure is logged but cannot change a ready dataset to failed. SES owns no activity content and receives only the destination address and generic completion text. The domain uses Easy DKIM records owned by Pulumi. While SES remains in its sandbox, recipients must also verify their address; production access is required before arbitrary approved Google users can receive notifications.
...
Published readers use the active tab's persisted longitude, latitude, and zoom verbatim. Dataset bounds are used only when a view has no explicitly supplied camera; they never overwrite a published camera after the manifest opens.
